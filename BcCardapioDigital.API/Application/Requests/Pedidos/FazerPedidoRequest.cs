...
namespace BcCardapioDigital.API.Application.Requests.Pedidos
{
    public record FazerPedidoRequest
    {
        ...
        {
            return new Pedido {TelefoneCliente = this.TelefoneCliente, NomeCliente = this.NomeCliente, FormaDePagamento = this.FormaDePagamento, Endereco = this.Endereco, Items = this.Items.Select(i => i.ToEntity()).ToList(), Data = this.Data, Status = Status.Processando };
        }
    }
